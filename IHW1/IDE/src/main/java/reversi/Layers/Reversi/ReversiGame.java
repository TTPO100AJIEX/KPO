...
public class ReversiGame extends Reversi {
    ...
    /*---------------------------Move---------------------------*/
    private final ArrayList<ReversiEmulator> history = new ArrayList<ReversiEmulator>();
    ...
    public Player getCurrentPlayer() {
        return this.isBlackMove ? this.black : this.white;
    }
    ...
    private void nextMove() {
        ...
        if (!this.hasAvailableMoves()) {
            ...
            if (this.consecutiveNoMovesAmount >= 2) {
                this.end();
                return;
            }
        } else {
    ...
    private void handleMove() {
        ...
        while (!move.isValid()) {
            Renderer.Context context = new Renderer.Context().addText("The move " + move + " is not allowed!", Renderer.Colors.RED).addLine(" Please specify a valid move!", Renderer.Colors.YELLOW);
            this.eventDispatcher.accept(new Render(context));
    ...
    private interface PlayerResult extends Renderer.Item {
        ...
        Renderer.Colors getStatusColor();

        String getPlayerName();

        Renderer.Colors getPlayerColor();

        int getScore();

        @Override
        default Renderer.Context getSubcontext() {
    ...
    public class BotRandom extends Player {
        public BotRandom(final String name) {
            super(name);
        }
    ...
    public class BotEasy extends Player {
        public BotEasy(final String name) {
            super(name);
        }
    ...
    public class BotAdvanced extends Player {
        public BotAdvanced(final String name) {
            super(name);
        }
    ...
    private final class Winner implements PlayerResult {
        ...
        public Renderer.Colors getPlayerColor() {
            return (ReversiGame.this.white == this.player) ? Renderer.Colors.YELLOW : Renderer.Colors.PURPLE;
        }

        @Override
        public int getScore() {
            return (ReversiGame.this.white == this.player) ? ReversiGame.this.whiteScore() : ReversiGame.this.blackScore();
        }
    }
