...
public final class Settings extends Layer implements Renderer.Item {
    public Settings(final Consumer<Event> eventDispatcher) {
    ...
    private void awaitAction() {
        ...
        switch (action.getValue()) {
            case 1: {
                this.eventDispatcher.accept(new SwitchControl(Parts.LEADERBOARD));
                break;
            }
            case 2: {
                this.eventDispatcher.accept(new SwitchControl(Parts.PLAY_PLAYER));
                break;
            }
            case 3: {
                this.eventDispatcher.accept(new SwitchControl(Parts.PLAY_EASY));
                break;
            }
            case 4: {
                this.eventDispatcher.accept(new SwitchControl(Parts.PLAY_ADVANCED));
                break;
            }
            case 5: {
                this.eventDispatcher.accept(new SwitchControl(Parts.PLAY_RANDOM));
                break;
            }
            case 6: {
                this.eventDispatcher.accept(new SwitchControl(Parts.QUIT));
                break;
            }
            default: {
