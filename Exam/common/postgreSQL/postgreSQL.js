...
{
    static #DefaultOptions = { parseInputDatesAsUTC: true, application_name: config.application.name };
    format = pg_format;

    #options; #pg_connection;
    ...
    #parse_pointers(data)
    {
        ...
        for (const key in data) JSONPointer.set(result, "/" + key, this.#parse_pointers(data[key]));
        return result;
    }
    ...
    async query(query, params, { one_response = false } = { })
    {
        ...
        const rows = this.#parse_pointers(data.rows);
    ...
    async query_multiple(queries = { })
    {
        ...
        const rows = this.#parse_pointers((Array.isArray(data) ? data : [ data ]).map(res => res.rows));
