...
export default class Interval extends PostgresInterval {
			...
			case "string": {
				if (Interval.#format.test(input)) {
					...
					while (input.length != 0) {
						const index = input.search(/y|d|h|m|s/),
							number = Number(input.slice(0, index));
						switch (input[index]) {
							case "y": {
								this.years += number;
								break;
	...
	toDays() {
		return this.toYears() * 365 + this.days;
	}
	toHours() {
		return this.toDays() * 24 + this.hours;
	}
	toMinutes() {
		return this.toHours() * 60 + this.minutes;
	}
	toSeconds() {
		return this.toMinutes() * 60 + this.seconds;
	}
	toMilliseconds() {
		return this.toSeconds() * 1000 + this.milliseconds;
	}
