...
async function register(app, options) {
	...
	await app.register(under_pressure, {
		...
		pressureHandler: (req, res, type, value) => {
			if (type === under_pressure.TYPE_HEAP_USED_BYTES) {
				console.warn(`Heap has been exhausted: ${value}`);
				return;
			}
			if (type === under_pressure.TYPE_RSS_BYTES) {
				console.warn(`RSS has been exhausted: ${value}`);
				return;
			}
			throw 503;
		},
	...
	await app.register(swagger, {
		openapi: {
			servers: [{ url: `https://${config.host}:${config.port}`, description: "Service" }],
		},
	});
