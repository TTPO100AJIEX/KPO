...
const name = new Date(), options = config.accounts.postgreSQL;
...
let command = ChildProcess.spawn('pg_dump', [ `--dbname=postgresql://${options.user}:${options.password}@${options.host}:${options.port}/${options.database}`, "-f", `${folder}/dumps/${filename}.sql` ]);
